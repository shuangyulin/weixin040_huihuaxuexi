<template>
	<view class="content">
		<form class="app-update-pv">
				...
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left"}' class="title">编号</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`zuoyeleixing`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
																																																																																																																																				if((!this.ruleForm.zuoyeleixing)){
					this.$utils.msg(`作业类型不能为空`);
					return
				}
																																				if(this.ruleForm.id){
					await this.$api.update(`zuoyeleixing`, this.ruleForm);
				}else{
					await this.$api.add(`zuoyeleixing`, this.ruleForm);
				}
